...
const TaskDetails = ({ visible, task, onInputChange, saveTeam, hideDialog, submitted }) => {
    return (
        <Dialog
            visible={visible}
            style={{ width: '32rem' }}
        ...
        >
            ...
            <InputText
                ...
                required
                className={submitted && !task?.createdBy?.name ? 'p-invalid' : ''}
            />
            {submitted && !task?.createdBy?.name && <small className="p-error">Admin name is required.</small>}
